 ...
 *------------------------------------------------------------------------
 */
status ascdate(uint32 now, /* Date and time in xinu format	*/
  ...
  if (Date.dt_daylight == DATE_DST_ON) {
    ...
    mar1 = (jan1 + Date.dt_msize[0] + Date.dt_msize[1] + isleap(year)) % 7;
  ...
  sprintf(str, "%3s  %3s %2d  %2d:%02d:%02d %s %d", Date.dt_dnam[dayofweek],
          Date.dt_mnam[month], day, hour, minute, second,
          dst ? dzones[TIMEZONE - 5] : zones[TIMEZONE - 5], year);
  return OK;
}
