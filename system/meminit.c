...
void meminit(void) {
  ...
  while (mmap_addr < mmap_addrend) {
    ...
    if ((uint32)maxheap <
        ((uint32)mmap_addr->base_addr + (uint32)mmap_addr->length)) {
      maxheap =
          (void *)((uint32)mmap_addr->base_addr + (uint32)mmap_addr->length);
    }
    ...
    if ((mmap_addr->base_addr <= (uint32)minheap) &&
        ((mmap_addr->base_addr + mmap_addr->length) > (uint32)minheap)) {
      ...
      next_block_length = (uint32)truncmb(mmap_addr->base_addr +
                                          mmap_addr->length - (uint32)minheap);
    } else {
