 ...
 *------------------------------------------------------------------------
 */
status lfscheck(struct lfdir *dirptr /* Ptr to an in-core directory	*/
) {
  uint32 reverse; /* LFS_ID in reverse byte order	*/

  /* Verify the File System ID, all 0's and all 1's fields */

  if ((dirptr->lfd_fsysid != LFS_ID) || (dirptr->lfd_allzeros != 0x00000000) ||
  ...
  reverse = ((LFS_ID >> 24) & 0x000000ff) | ((LFS_ID >> 8) & 0x0000ff00) |
