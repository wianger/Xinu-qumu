 ...
 *------------------------------------------------------------------------
 */
devcall ttyinit(struct dentry *devptr /* Entry in device switch table	*/
) {
  ...
  /* Set baud rate */
  outb((uint32)&uptr->lcr, UART_LCR_DLAB);
  outb((uint32)&uptr->dlm, 0x00);
  outb((uint32)&uptr->dll, 0x18);

  outb((uint32)&uptr->lcr, UART_LCR_8N1); /* 8 bit char, No Parity, 1 Stop*/
