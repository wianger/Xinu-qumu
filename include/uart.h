 ...
 */
struct uart_csreg {
  volatile uint8 buffer; /* receive buffer (when read)		*/
                         /*   OR transmit hold (when written)	*/
  volatile uint8 ier;    /* interrupt enable			*/
  volatile uint8 iir;    /* interrupt identification (when read)	*/
                         /*   OR FIFO control (when written)	*/
  volatile uint8 lcr;    /* line control register		*/
  volatile uint8 mcr;    /* modem control register		*/
  volatile uint8 lsr;    /* line status register			*/
  volatile uint8 msr;    /* modem status register		*/
  volatile uint8 scr;    /* scratch register			*/
};
